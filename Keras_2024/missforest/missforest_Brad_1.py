...
class MissForest:
    ...
    def compute_gamma_categorical_Brad(self, x_imp_cat: list, x_imp_cat_prev: list
                                  ) -> np.ndarray | int:
        """
        Compute and returns Gamma of categorical variables in imputed 'x'.
        ...
        int
            ...
            imputed 'x'.
        """
        print ('compute_gamma_categorical()')
        print ('len(self.categorical) =', len(self.categorical))
...
#        print ('all_x_imp_cat', len(all_x_imp_cat), all_x_imp_cat[-1].shape)

        x_imp_prev = x_imp.copy() # (Brad)
	
        n_iter = 0
        while True:
            for c in missing_rows:
                print ('Iteration ', n_iter, ' of ', self.max_iter,  ', feature ', c) # Added by Brad
                if c in mappings:
		...
		# (Brad) x_imp is the imputed dataframe
		# (Brad) all_x_imp_cat is a list of the x_imp dataframes with just the categorical features
		# (Brad) I un-indented the last two lines so that it only appends x-imp once every iteration, not for every feature
		# (Brad) The only reason for this all_x_imp_cat and all_gamma_cat is to test whether the imputation is diverging rather than converging, so it can break and output.  
#            all_x_imp_cat.append(
#                    x_imp[self.categorical].reset_index(drop=True))
#            all_x_imp_num.append(
#                    x_imp[self.numerical].reset_index(drop=True))
#            print ('all_x_imp_cat', len(all_x_imp_cat), all_x_imp_cat[-1].shape)
            ...
            all_gamma_cat.append(self.compute_gamma_categorical_Brad(
                    x_imp[self.categorical].reset_index(drop=True),
                    x_imp_prev[self.categorical].reset_index(drop=True)
            ))
...
#            all_gamma_num.append(self.compute_gamma_numerical(all_x_imp_num))
            print ('all_gamma_cat = ', all_gamma_cat)

            n_iter += 1
#            if n_iter > self.max_iter:
            if n_iter >= self.max_iter: # (Brad)
                break
            ...
            if (
                ...
                x_imp = x_imp_prev # (Brad) If it starts to diverge, take the previous one and stop.
                break
